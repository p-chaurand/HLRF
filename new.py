    ...
    def run(
        ...
        **kwargs: Any,
        ...
        def is_adversarial(perturbed: ep.Tensor, classes: ep.Tensor) -> ep.Tensor:
            ...
            print(perturbed.min(), perturbed.max())
            if not (targeted):
                return (
        ...
        inputs_ = to_attack_space(inputs_)
        delta = ep.zeros_like(inputs_)#Rent
        found_advs = ep.full_like(classes, fill_value=False)
        found_advs = found_advs != 0
        self.sigma = ep.full(inputs_, (N,), 1)
        print("inputs", inputs_.min(), inputs_.max())
        for step in range(self.steps):
            self.queries += 1
            _, loss, gradient = loss_aux_and_grad(to_model_space(delta + inputs_))
        ...
        best_advs = to_model_space(best_advs)
        print(best_advs.flatten().min())
        print(best_advs.flatten().max())
