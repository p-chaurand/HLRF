...
class iHL_RFAttack(MinimizationAttack):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        best_advs = copy.deepcopy(inputs_)
        ...
        for step in range(self.steps):
            ...
            is_advs = is_adversarial(advs, classes)
            found_advs = ep.logical_or(found_advs, is_advs)

            norms = ep.astensor((advs - inputs_).raw.flatten(1).norm(dim=1))
            closer = norms < best_advs_norms
            new_best = ep.logical_and(closer, is_advs)

            new_best_ = atleast_kd(new_best, best_advs.ndim)

            best_advs = ep.where(new_best_, advs, best_advs)
            ...
            if self.max_queries is not None and all(self.queries > self.max_queries):
                break
        print(best_advs.min(), best_advs.max())
